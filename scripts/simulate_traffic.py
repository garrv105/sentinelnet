...
try:
    from scapy.all import IP, TCP, UDP, ICMP, DNS, DNSQR, Raw, wrpcap, Packet
    SCAPY_AVAILABLE = True
except ImportError:
    SCAPY_AVAILABLE = False

from sentinelnet.core.packet_capture import PacketRecord, Protocol
from sentinelnet.core.event_bus import EventBus, ThreatEvent
...
class TrafficSimulator:
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "time_span_secs": max(p.timestamp for p in self._packets) - min(p.timestamp for p in self._packets) if self._packets else 0,
        }
